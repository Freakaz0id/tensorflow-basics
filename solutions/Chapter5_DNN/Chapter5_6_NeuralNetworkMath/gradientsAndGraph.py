...
def get_gradients(
    ...
    grad_var_tuples = [(g, w) for (g, w) in zip(grads, model.trainable_variables)]
...
def main() -> None:
    ...
    model.compile(loss="mse", optimizer=Adam(learning_rate=1e-2), metrics=["mse"])

    tb_callback = TensorBoard(log_dir=MODEL_LOG_DIR, embeddings_freq=0, write_graph=True)

    model.fit(x=x, y=y, verbose=1, batch_size=1, epochs=0, callbacks=[tb_callback])
